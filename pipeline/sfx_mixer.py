    ...
    return json.dumps(
        {
            ...
            "rules": [
                ...
                "Default to subtle Human sounds for ordinary dialogue, because the input is a generated conversation without reliable real-world scene audio.",
                "Good default Human sounds include quiet laughter, breath, sighs, coughs, small non-speech reactions, and light body or clothing movement when the text supports them.",
                "Use non-human categories only when the dialogue text explicitly grounds them, such as a mentioned place, object, activity, animal, weather condition, vehicle, sport crowd, machine, phone, door, or music.",
                "Do not invent broad ambience or off-screen events just to make the mix busy; if there is no clear textual cue, choose a restrained Human sound or no event.",
                "Use at most max_events events.",
                "Prefer placing events in dialogue gaps. If an event overlaps speech, keep gain_db quiet.",
                "Avoid Music, Animal, Natural sounds, Source-ambiguous sounds, and loud impacts unless the dialogue explicitly calls for that kind of sound.",
            ...
            "selection_policy": {
                "default_bias": "Prefer Human sounds for conversational reactions and small speaker-adjacent details.",
                "non_human_allowed_when": "The turn text gives a concrete cue for the sound source or scene.",
                "silence_is_ok": "Return fewer than max_events if extra sounds would feel ungrounded.",
            },
